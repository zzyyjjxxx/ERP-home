        ...
        <>
          {record.status === 0 && (
            <>
              ...
              <PermissionBtn permission="sales:return:audit" type="link" onClick={async () => { await completeReturn(record.id); message.success('完成'); setTableKey(k => k + 1); }}>完成</PermissionBtn>
            </>
          )}
          {record.status === 1 && <PermissionBtn permission="sales:return:unaudit" type="link" onClick={async () => { await unauditReturn(record.id); message.success('反审核成功'); setTableKey(k => k + 1); }}>反审核</PermissionBtn>}
